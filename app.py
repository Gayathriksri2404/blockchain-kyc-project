...
import pytz  # Added for IST timestamps
...
def get_next_user_folder():
    existing = [d for d in os.listdir(UPLOAD_FOLDER) if os.path.isdir(os.path.join(UPLOAD_FOLDER, d)) and d.startswith("user_")]
    if not existing:
        next_index = 1
    else:
        numbers = []
...
def bank_login():
    ...
    if request.method == "POST":
        ...
        if bank_id=="admin" and password=="1234":
            ...
            if next_page=="view_kyc": return redirect(url_for("kyc_records_table"))
            elif next_page=="kyc": return redirect(url_for("kyc"))
            elif next_page=="verify": return redirect(url_for("verify"))
            elif next_page=="blockchain": return redirect(url_for("blockchain"))
...
def fl_round(round_num):
    ...
    return jsonify({'round':round_num,'client_updates':client_updates,'global_model':global_model})
        ...
        if current_block.get("previous_hash")!=previous_block.get("hash"):
            ...
            for record in kyc_records:
                if record.get("hash")==kyc_hash:
                    ...
                    break
            return {"status":False,"tampered_index":current_block.get("index"),"kyc_hash":kyc_hash,"kyc_name":kyc_name}
    return {"status":True}
